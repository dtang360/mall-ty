...
import cn.wemalls.core.filter.HibernatePropertyAllFilter;
...
public class WxCommonUtil {
    ...
    public static void printObjJson(Object resMap,HttpServletResponse response) {
        try {
        	HibernatePropertyFilter filter = new HibernatePropertyFilter();
//        	filter.apply(null, "", null);
        	String json = JSON.toJSONString(resMap, filter);
//        	String json = JSON.toJSONString(resMap);
    ...
    public static void printObjJsonAll(Object resMap,HttpServletResponse response,String[] propertys,Boolean collectionFlag,Boolean lazyFlag,Boolean nullFlag) {
        try {
        	HibernatePropertyAllFilter filter = new HibernatePropertyAllFilter();
        	filter.setPropertyNames(propertys);
        	filter.setCollectionFlag(collectionFlag);
        	filter.setLazyFlag(lazyFlag);
        	filter.setNullFlag(nullFlag);
        	String json = JSON.toJSONString(resMap,filter,SerializerFeature.DisableCircularReferenceDetect);
//        	String json = JSON.toJSONString(resMap);
        	response.setContentType("text/plain");
        	response.setHeader("Cache-Control", "no-cache");
        	response.setCharacterEncoding("UTF-8");
