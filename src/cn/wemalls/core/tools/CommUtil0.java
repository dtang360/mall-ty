...
public class CommUtil0 {
        ...
        if (lists != null && lists.size() > 0){
            for(int i = 0; i < lists.size(); i++){
                ...
                if((Boolean)map.get("IsSecondDomainOpen")){
                    goods_url = "https://" + goods.getGoods_store().getStore_second_domain() + "." + map.get("domainPath") + "/goods_" + goods.getId() + ".htm";
                }
        ...
        if (lists != null && lists.size() > 0){
            for(int i = 0; i < lists.size(); i++){
                ...
                for(GoodsCart gc : obj.getGcs()){
                    ...
                    if((Boolean)map.get("IsSecondDomainOpen")){
                        goods_url = "https://" + gc.getGoods().getGoods_store().getStore_second_domain() + "." + map.get("domainPath") + "/goods_" + gc.getGoods().getId() + ".htm";
                    }
    ...
    public static void saveWebPaths(Map<String, Object> map, SysConfig config, HttpServletRequest request){
        ...
        if ((config.isSecond_domain_open()) && (!CommUtil.generic_domain(request).equals("localhost"))){
            webPath = "https://www." + CommUtil.generic_domain(request) + port + contextPath;
        }
