...
import cn.wemalls.core.tools.*;
import cn.wemalls.core.tools.bean.WxOauth2Token;
import cn.wemalls.core.tools.bean.WxToken;
...
import cn.wemalls.manage.admin.tools.PaymentTools;
import cn.wemalls.manage.seller.tools.TransportTools;
import cn.wemalls.pay.alipay.config.AlipayConfig;
import cn.wemalls.pay.alipay.util.AlipaySubmit;
import cn.wemalls.pay.tools.PayTools;
...
public class CartViewAction {
    ...
    @Autowired
    private StorePrinterApiService printerApiService;

    private static Logger logger = LoggerFactory.getLogger(CartViewAction.class);

    //购物车数据
    private List<StoreCart> cart_calc_wxapplet(HttpServletRequest request,boolean remove){
                ...
                if (user.getStore() != null){
                    ...
                    for (StoreCart sc : store_cookie_cart){
                        ...
                        for (GoodsCart gc : ((StoreCart) sc).getGcs()){
                            gc.getGsps().clear();
                            this.goodsCartService.executeNativeSQL(" delete from wemalls_goodscart  where id = "+CommUtil.null2Long(gc.getId()));
...
//                            this.goodsCartService.delete(gc.getId());
    ...
    //被勾选购物车商品
    private List<StoreCart> cart_checked_calc(HttpServletRequest request){
        List<StoreCart> cart = new ArrayList<StoreCart>();
        List<StoreCart> user_cart = new ArrayList<StoreCart>();
        List<StoreCart> cookie_cart = new ArrayList<StoreCart>();
        User user = null;
...
//            }else{
                ...
                params.put("sc_status", Integer.valueOf(0));
                user_cart = this.storeCartService.query(
                                "select obj from StoreCart obj where obj.user.id=:user_id and checked=true and obj.sc_status=:sc_status", params,
                                -1, -1);
//            }
...
//        }
    ...
    //直接购买商品
    private List<StoreCart> cart_calc1(HttpServletRequest request,String id){

    	List<StoreCart> cart = new ArrayList<StoreCart>();
    	Goods goods=null;
    	goods=goodsService.getObjById(CommUtil.null2Long(id));
    ...
    public ModelAndView cart_menu_detail(HttpServletRequest request, HttpServletResponse response){
        ...
        for (GoodsCart gc : list){
            ...
            if (CommUtil.null2String(gc.getCart_type()).equals("combin"))
                ...
                total_price = CommUtil.null2Float(Double.valueOf(CommUtil.mul(Float.valueOf(gc.getCount()), gc.getPrice()))) + total_price;
            }
        }
...
//                    add = false;
        ...
        //清空购物店铺信息，重新加载
        for (int i=0;i<cart.size();i++){
        	 GoodsCartQueryObject qo = new GoodsCartQueryObject();
             qo.addQuery("obj.sc.id",
                         new SysMap("storecart_id",((StoreCart)cart.get(i)).getId()), "=");
             qo.setPageSize(Integer.valueOf(9999));
        	((StoreCart)cart.get(i)).setGcs(this.goodsCartService.list(qo).getResult());
        }
        ...
        if (add){// 买家当前所选规格的商品可以添加到购物车
            ...
            if (CommUtil.null2String(buy_type).equals("")){
                ((GoodsCart) obj).setCount(Float.valueOf(count));
        ...
        for (Object type = cart.iterator(); ((Iterator) type).hasNext();){
            ...
            for (obj = sc1.getGcs().iterator(); ((Iterator) obj).hasNext();){
                ...
                total_price = total_price + CommUtil.mul(gc1.getPrice(),gc1.getCount());
            }
        }
     ...
     * @param buy_type
     */
    @RequestMapping({ "/wxapplet/add_goods_cart.htm" })
    public void wxapplet_add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count,
                               String price, String gsp, String buy_type){
        String cart_session_id = "";
        Cookie[] cookies = request.getCookies();
        if (cookies != null){
...
//                    add = false;
     ...
     * @param store_id
     */
    @RequestMapping({ "/wxapplet/remove_goods_cart.htm" })
    public void wxapplet_remove_goods_cart(HttpServletRequest request, HttpServletResponse response, String id,
                                  String store_id){
        GoodsCart gc = this.goodsCartService.getObjById(CommUtil.null2Long(id));
        StoreCart the_sc = gc.getSc();
        gc.getGsps().clear();
     ...
     * @param store_id
     */
    @RequestMapping({ "/wxapplet/checked_goods_cart.htm" })
    public void wxapplet_checked_goods_cart(HttpServletRequest request, HttpServletResponse response, String ids,
                                  Boolean checked){
    	String[] strArr = ids!=null&&ids.length()>0?ids.split(","):null;
    	if(strArr!=null&&strArr.length>0) {
    		for (String id : strArr) {
    			if(id!=null&&id.length()>0) {
    				...
    				gc.getGsps().clear();
    				if(checked!=null) {
    					gc.setChecked(checked);
    				}else {
    					return;
    				}
    				this.goodsCartService.update(gc);
    				//StoreCart 是否选中，重新赋值
    				StoreCart sc=gc.getSc();
    				sc.setChecked(false);
...
//        map.put("sc_total_price", Double.valueOf(sc_total_price));
    ...
    @RequestMapping({ "/wxapplet/goods_count_adjust.htm" })
    public void wxapplet_goods_count_adjust(HttpServletRequest request, HttpServletResponse response, String cart_id,
        ...
        if (cart_type.equals("")){
            if (goods.getGroup_buy() == 2){
                ...
                if (gg.getGg_count() >= CommUtil.null2Float(count))
                            ...
                            if (art.getId().toString().equals(cart_id)){
                                ((StoreCart) sc1).setTotal_price(
                                    BigDecimal.valueOf(CommUtil.add(((StoreCart) sc1).getTotal_price(),
                                                                    Double.valueOf((CommUtil.null2Float(count) - art.getCount())
                                                                            * CommUtil.null2Double(art.getPrice())))));
                                art.setCount(CommUtil.null2Float(count));
                                gc1 = art;
            ...
            }else if (goods.getGoods_inventory() >= CommUtil.null2Float(count)){
                    ...
                    for (int i = 0; i < scart.getGcs().size(); i++){
                        ...
                        if (gcart.getId().toString().equals(cart_id)){
                            scart.setTotal_price(BigDecimal.valueOf(CommUtil.add(scart.getTotal_price(),
                                                                    Double.valueOf((CommUtil.null2Float(count) - gcart.getCount())
                                                                            * Double.parseDouble(gcart.getPrice().toString())))));
                            gcart.setCount(CommUtil.null2Float(count));
                            gc1 = gcart;
        ...
        if (cart_type.equals("combin")){
            if (goods.getGoods_inventory() >= CommUtil.null2Float(count))
                    ...
                    for (int i = 0; i < sscart.getGcs().size(); i++){
                        ...
                        if (((GoodsCart) gc).getId().toString().equals(cart_id)){
                            sscart.setTotal_price(BigDecimal.valueOf(CommUtil.add(sscart.getTotal_price(),
                                                  Float.valueOf((CommUtil.null2Float(count) - ((GoodsCart) gc).getCount())
                                                                * CommUtil.null2Float(((GoodsCart) gc).getGoods().getCombin_price())))));
                            ((GoodsCart) gc).setCount(CommUtil.null2Float(count));
                            gc1 = (GoodsCart) gc;
...
//        ((Map) map).put("goods_total_price", Double.valueOf(df.format(goods_total_price)));
    ...
    public ModelAndView goods_cart1(HttpServletRequest request, HttpServletResponse response){
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/goods_cart1.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/goods_cart1.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        List<StoreCart> cart = cart_calc(request);
        if (cart != null){
...
//                            this.goodsCartService.delete(gc.getId());
     ...
     */
    @SecurityMapping(display = false, rsequence = 0, title = "查看购物车", value = "/goods_cart1.htm*", rtype = "buyer", rname = "购物流程1", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/wxapplet/goods_cart1.htm" })
    public void wxapplet_goods_cart1(HttpServletRequest request, HttpServletResponse response){
    	Map resMap=new HashMap();
        List<StoreCart> cart = cart_calc_wxapplet(request,false);
        if (cart != null){
            Store store = SecurityUserHolder.getCurrentUser().getStore() != null
                          ? SecurityUserHolder.getCurrentUser().getStore() : null;
            if (store != null){
...
//                            this.goodsCartService.delete(gc.getId());
            ...
            request.getSession(false).setAttribute("cart", cart);
            resMap.put("objsList", cart);
            resMap.put("goodsViewTools", this.goodsViewTools);
        }else{
        	resMap.put("op_title", "购物车信息为空");
        }
        if (this.configService.getSysConfig().isZtc_status()){
            List ztc_goods = null;
            Map ztc_map = new HashMap();
        ...
        String[] propertys={"objsList","StoreCart","gcs","goods","goods_main_photo"};
    ...
    public ModelAndView goods_cart2(HttpServletRequest request, HttpServletResponse response){
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/goods_cart2.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/goods_cart2.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        List<StoreCart> carts = cart_calc(request);
//        StoreCart sc = null;
//        if (cart != null){
//            for (StoreCart sc1 : cart){
...
//        }
        if (carts != null){
            ...
            List addrs = this.addressService.query(
                             "select obj from Address obj where obj.user.id=:user_id and id!=0 order by obj.addTime desc", params, -1, -1);
...
//            }
            ...
            params.clear();
            BigDecimal coupon_order_amount=new BigDecimal(0.0);
            for (StoreCart sc : carts) {
            	coupon_order_amount=coupon_order_amount.add(sc.getTotal_price());
			}
            params.put("coupon_order_amount",coupon_order_amount);
            params.put("user_id", SecurityUserHolder.getCurrentUser().getId());
...
//            mv.addObject("goods_delivery", Boolean.valueOf(goods_delivery));
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "确认购物车填写地址", value = "/goods_cart2.htm*", rtype = "buyer", rname = "购物流程2", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/wxapplet/goods_cart2.htm" })
    public void wxGoods_cart2(HttpServletRequest request, HttpServletResponse response,String id){
    	Map resMap=new HashMap();
    	List<StoreCart> carts=new ArrayList<StoreCart>();
    	//直接购买
    	if(id!=null&&!"".equals(id)&&id.length()>0) {
    		carts=cart_calc1(request,id);
    	//选中购买	
    	}else {
    		carts = cart_calc_wxapplet(request, true);
    	}
...
//            }
            ...
            List couponinfos = this.couponInfoService.query(
                                   "select obj from CouponInfo obj where obj.coupon.coupon_order_amount<=:coupon_order_amount and obj.status=:status and obj.user.id=:user_id and obj.coupon.coupon_begin_time<=:coupon_begin_time and obj.coupon.coupon_end_time>=:coupon_end_time",
                                   params, -1, -1);
            resMap.put("couponinfos", couponinfos);
            resMap.put("scs",carts);
            resMap.put("cart_session", cart_session);
//            mv.addObject("store_id", store_id);
        ...
        String[] propertys={"objsList","scs","StoreCart","store","gcs","GoodsCart","goods","goods_main_photo","addrs","Address","area","parent","couponinfos","cart_session"};
    ...
    public ModelAndView goods_cart3(HttpServletRequest request, HttpServletResponse response, String cart_session,
                                    String store_id, String addr_id, String coupon_id,String delivery_way,String payType,String pick_store) throws Exception {
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/goods_cart3.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/goods_cart3.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        ...
        if (cart != null){
            if (CommUtil.null2String(cart_session1).equals(cart_session)){
                ...
                Address addr=null;
              //送货
                if(delivery_way!=null&&delivery_way.equals("0")){
                ...
                }else if(delivery_way!=null&&delivery_way.equals("1")){
                	//自提
                	addr=this.addressService.getObjById(CommUtil.null2Long("0"));
                	of.setPick_store(pick_store);
                	of.setTotalPrice(of.getGoods_amount());
                }
                of.setAddr(addr);
...
//                    if (sc.getStore().getId().toString().equals(store_id)){
                        for (Iterator localIterator2 = sc.getGcs().iterator(); localIterator2.hasNext();){
                            gc = (GoodsCart) localIterator2.next();
                            gc.setOf(of);
                            gc.getGoods().setGoods_inventory(gc.getGoods().getGoods_inventory()-gc.getCount());
                            this.goodsCartService.update(gc);
                        }
...
//                }
                ...
                ofl.setLog_user(SecurityUserHolder.getCurrentUser());
                this.orderFormLogService.save(ofl);
                resMap.put("orderfrom", of);
                resMap.put("paymentTools", this.paymentTools);
                //发送邮件
                if (this.configService.getSysConfig().isEmailEnable()){
                    send_email(request, of, of.getUser().getEmail(), "email_tobuyer_order_submit_ok_notify");
                }
...
//            }else{
//            	resMap.put("op_title", "订单已经失效");
//            	resMap.put("url", CommUtil.getURL(request) + "/index.htm");
//            }
        }else{
        	resMap.put("op_title", "订单信息错误");
        	resMap.put("url", CommUtil.getURL(request) + "/index.htm");
        }

        String[] propertys={"orderfrom","op_title"};
        WxCommonUtil.printObjJsonAll(resMap, response,propertys,false,false,false);
    }
    ...
    public ModelAndView order_pay_view(HttpServletRequest request, HttpServletResponse response, String id){
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/order_pay.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/order_pay.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
     ...
     */
    @SecurityMapping(display = false, rsequence = 0, title = "订单支付详情", value = "/order_pay_view.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/wxapplet/order_pay_view.htm" })
    public void wxapplet_order_pay_view(HttpServletRequest request, HttpServletResponse response, String id){
    	  Map resMap=new HashMap();
    	
        OrderForm of = this.orderFormService.getObjById(CommUtil.null2Long(id));
        if (of!=null&&of.getOrder_status() == 10||of.getOrder_status() == 40){
            resMap.put("orderfrom", of);
//            resMap.put("paymentTools", this.paymentTools);
//            resMap.put("url", CommUtil.getURL(request));
        }else if (of!=null&&of.getOrder_status() < 10){
        	resMap.put("op_title", "该订单已经取消！");
        	resMap.put("url", CommUtil.getURL(request) + "/index.htm");
        }else{
        	resMap.put("op_title", "该订单已经付款！");
        	resMap.put("url", CommUtil.getURL(request) + "/index.htm");
        }
...
//            configMap.put("url", clientUrl);
    ...
    public ModelAndView order_pay_outline(HttpServletRequest request, HttpServletResponse response, String payType,
      ...
      if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
          mv = new JModelAndView("android/success.html", this.configService.getSysConfig(),
                                 this.userConfigService.getUserConfig(), 1, request, response);
      }
      if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
          mv = new JModelAndView("ios/success.html", this.configService.getSysConfig(),
                                 this.userConfigService.getUserConfig(), 1, request, response);
      }
...
//        }
    ...
    public Boolean orderFormPrint(OrderForm of){
    	//标签说明：
		//单标签: 
		...
		String content;
		content = "<CB>新疆果蔬配送</CB><BR>";
		content += "名称　　　　　 单价  数量 金额<BR>";
		content += "--------------------------------<BR>";
		List<GoodsCart> carts=of.getGcs();
		GoodsCart cart=new GoodsCart();
		String cc="                                 ";
		if(carts!=null&&carts.size()>0){
			for(int i=0;i<carts.size();i++){
				cart=carts.get(i);
				content += (cart.getGoods().getGoods_name()+cc).substring(0, 9)+" "+(cart.getGoods().getGoods_current_price()+cc).substring(0, 6)+" "+(String.valueOf(cart.getCount())+cc).substring(0, 4)+" "+(cart.getGoods().getGoods_current_price().multiply(new BigDecimal(Float.toString(cart.getCount())))).setScale(2,BigDecimal.ROUND_HALF_UP)+"<BR>";
			}
		}
		content += "--------------------------------<BR>";
		content += "合计："+of.getTotalPrice()+"元<BR>";
		if(of.getAddr()!=null){
			content += "收货人："+of.getAddr().getTrueName()+"<BR>";
			content += "送货地点："+of.getAddr().getArea_info()+"<BR>";
			content += "联系电话："+of.getAddr().getMobile()+"<BR>";
		}else{
			content += "收货人：<BR>";
			content += "送货地点：<BR>";
			content += "联系电话：<BR>";
		}
		content += "订餐时间："+of.getAddTime()+"<BR>";
		content += "备注："+of.getMsg()+"<BR>";
		content += "<BOLD>签字：</BOLD><BR>";
		content += "<BR>";
		content += "<BR>";
		content += "<BR>";
		content += "<QR>http://m.pifashang365.com</QR>";
		printerApiService.print(of,"817502189", content, "1");
        return true;
    }
...
//            params.put("store_id", of.getStore().getId());
                    ...
                    if (goods.getInventory_type().equals("all")){
                        ...
                        for (Iterator localIterator4 = list.iterator(); localIterator4.hasNext();){
                            ...
                            if (Arrays.equals(temp_ids, gsp_list)){
                                temp.put("count",
                                         Float.valueOf((Float)temp.get("count") - gc.getCount()));
                            }
                        }
    ...
    public ModelAndView order_finish(HttpServletRequest request, HttpServletResponse response, String order_id){
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/order_finish.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/order_finish.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "订单支付结果", value = "/order_finish.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/wxapplet/order_finish.htm" })
    public ModelAndView wxapplet_order_finish(HttpServletRequest request, HttpServletResponse response, String order_id){
        ModelAndView mv = new JModelAndView("order_finish.html", this.configService.getSysConfig(),
                                            this.userConfigService.getUserConfig(), 1, request, response);
        String wemalls_view_type = CookitTools.Get_View_Type(request);
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("wap"))){
    ...
    public ModelAndView cart_address(HttpServletRequest request, HttpServletResponse response, String id, String store_id){
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/cart_address.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/cart_address.html", this.configService.getSysConfig(),
                                   this.userConfigService.getUserConfig(), 1, request, response);
        }
    ...
    public ModelAndView address(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy, String orderType, String store_id){
        ...
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("android"))){
            mv = new JModelAndView("android/address.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
        if((wemalls_view_type != null) && (!wemalls_view_type.equals("")) && (wemalls_view_type.equals("ios"))){
            mv = new JModelAndView("ios/address.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
        }
    ...
    //编辑收货地址
    @SecurityMapping(display = false, rsequence = 0, title = "修改收货地址", value = "/address_edit.htm*", rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({"/wxapplet/address_area.htm"})
    public void wxapplet_address_area(HttpServletRequest request, HttpServletResponse response){
    	Map resMap=new HashMap();
        List areas = this.areaService.query("select obj from Area obj where obj.parent.id is null", null, -1, -1);
        resMap.put("areas", areas);       
        String[] propertys={"areas","childs"};
        WxCommonUtil.printObjJsonAll(resMap, response,propertys,false,false,false);
    }
    
    //编辑收货地址
    @SecurityMapping(display = false, rsequence = 0, title = "修改收货地址", value = "/address_edit.htm*", rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({"/wxapplet/address_edit.htm"})
    public void wxapplet_address_edit(HttpServletRequest request, HttpServletResponse response, String id, String currentPage, String store_id){
    	Map resMap=new HashMap();
        List areas = this.areaService.query("select obj from Area obj where obj.parent.id is null", null, -1, -1);
        Address obj = this.addressService.getObjById(CommUtil.null2Long(id));
        //获取编辑时地址上下层关系
        List objAreaLevel=new ArrayList();
        if(null!=obj.getArea()) {
        	if(null!=obj.getArea().getParent()) {
        		if(null!=obj.getArea().getParent().getParent()) {
        			objAreaLevel.add(obj.getArea().getParent().getParent().getId());
        		}
        		objAreaLevel.add(obj.getArea().getParent().getId());
        	}
        	objAreaLevel.add(obj.getArea().getId());
        }
        resMap.put("objAreaLevel", objAreaLevel);
        resMap.put("obj", obj);
        resMap.put("areas", areas);
        resMap.put("store_id", store_id);
        resMap.put("currentPage", currentPage);        
        String[] propertys={"obj","objAreaLevel","areas","childs","store_id","currentPage"};
        WxCommonUtil.printObjJsonAll(resMap, response,propertys,false,false,false);
    }
    
    @SecurityMapping(display = false, rsequence = 0, title = "购物车中收货地址保存", value = "/cart_address_save.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/wxapplet/address_save.htm" })
    public void wxapplet_cart_address_save(HttpServletRequest request, HttpServletResponse response, String id, String area_id, String store_id){
        WebForm wf = new WebForm();
        Address address = null;
        ...
        //判断更新用户前默认地址
        if(address.isDefault_flag()) {
        	 this.addressService.UpdateObjsToNotUserDefault(SecurityUserHolder.getCurrentUser().getId());
        }
        ...
        Map resMap=new HashMap();
        String[] propertys={"obj","areas","store_id","currentPage"};
        WxCommonUtil.printObjJsonAll(resMap, response,propertys,false,false,false);
    }
    
    //删除地址
    @SecurityMapping(display = false, rsequence = 0, title = "收货地址删除", value = "/address_del.htm*", rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
    @RequestMapping({"/wxapplet/address_del.htm"})
    public void wxapplet_address_del(HttpServletRequest request, HttpServletResponse response, String mulitId, String currentPage, String store_id){
        String[] ids = mulitId.split(",");
        for (String id : ids){
            if (!id.equals("")){
                Address address = this.addressService.getObjById(Long.valueOf(Long.parseLong(id)));
        ...
        Map resMap=new HashMap();
        resMap.put("status","1");
        String[] propertys={"status","obj","areas","store_id","currentPage"};
        WxCommonUtil.printObjJsonAll(resMap, response,propertys,false,false,false);
    }
