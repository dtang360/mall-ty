...
public class UCViewAction {
    ...
    private String uc_answer(HttpServletRequest request, HttpServletResponse response){
        ...
        if (action.equals("synlogin")){
            if (!API_SYNLOGIN)
                return API_RETURN_FORBIDDEN;
            wemalls_login(request, response, get);
        }else if (action.equals("synlogout")){
            if (!API_SYNLOGOUT)
                return API_RETURN_FORBIDDEN;
            wemalls_logout(request, response, get);
        }else{
            ...
            if (action.equals("updatepw")){
                if (!API_UPDATEPW)
                    return API_RETURN_FORBIDDEN;
                wemalls_update_pws(request, response, get);
                return API_RETURN_SUCCEED;
            }
    ...
    protected void wemalls_login(HttpServletRequest request, HttpServletResponse response, Map<String, String> args){
        ...
        if (!admin_login){
            ...
            String url = CommUtil.getURL(request) +
                         "/wemalls_login.htm?username=" +
                         CommUtil.encode(userName) + "&password=" +
                         "wemalls_thid_login_" + password + "&encode=true";
            try {
    ...
    protected void wemalls_logout(HttpServletRequest request, HttpServletResponse response, Map<String, String> args){
        String url = CommUtil.getURL(request) + "/wemalls_logout.htm";
        try {
            response.sendRedirect(url);
        } catch (IOException e){
            e.printStackTrace();
    ...
    protected void wemalls_update_pws(HttpServletRequest request, HttpServletResponse response, Map<String, String> args){
