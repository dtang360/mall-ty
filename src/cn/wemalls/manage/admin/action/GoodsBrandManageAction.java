...
import cn.wemalls.foundation.domain.GoodsType;
import cn.wemalls.foundation.domain.SysConfig;
import cn.wemalls.foundation.domain.query.GoodsBrandQueryObject;
import cn.wemalls.foundation.service.IAccessoryService;
import cn.wemalls.foundation.service.IGoodsBrandCategoryService;
import cn.wemalls.foundation.service.IGoodsBrandService;
...
public class GoodsBrandManageAction {
    ...
    @SecurityMapping(display = false, rsequence = 0, title = "商品品牌AJAX更新", value = "/admin/goods_brand_deleteable.htm*", rtype = "admin", rname = "品牌管理", rcode = "goods_brand", rgroup = "商品")
    @RequestMapping({"/admin/goods_brand_deleteable.htm"})
    public void deleteable(HttpServletRequest request, HttpServletResponse response, String id, String fieldName, String value) throws ClassNotFoundException {
    	 GoodsBrand brand = this.goodsBrandService.getObjById(
                 Long.valueOf(Long.parseLong(id)));
    	 try {
    		 ...
    		 if(brand.getGoods_list()!=null&&brand.getGoods_list().size()>0){
    			 writer.print("cancel");
    		 }else{
