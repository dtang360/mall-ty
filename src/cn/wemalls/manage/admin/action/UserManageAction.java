        ...
        if (condition != null){
            ...
            if (condition.equals("trueName")){
                ...
                .addQuery("obj.trueName",
                          new SysMap("trueName", "%" +value + "%"), "like");
            }
        }
...
//    @RequestMapping({"/admin/storeclass_delable.htm"})
//    public void deleteable(HttpServletRequest request, HttpServletResponse response, String mulitId, String fieldName, String value) throws ClassNotFoundException {
//    	String[] ids = mulitId.split(",");
...
//        for (String id : ids){
//            if (!id.equals("")){
//                Set list = genericIds(this.storeclassService.getObjById(Long.valueOf(Long.parseLong(id))));
//                Map params = new HashMap();
//                params.put("ids", list);
//                List<StoreClass> scs = this.storeclassService.query("select obj from StoreClass obj where obj.parent.id in (:ids) order by obj.level desc",
//                                           params, -1, -1);
//                params.clear();
//                params.put("ids", list);
//                List<Store> sl = this.storeService.query("select obj from Store obj where obj.sc.id in (:ids) ",params, -1, -1);
//	            try {
//	                PrintWriter writer = response.getWriter();
//		            if(scs!=null&&scs.size()>0){
//		            	writer.print("选择分类关联子分类信息，无法删除！");
//		            	return;
//		            }
//		            if(sl!=null&&sl.size()>0){
//		            	writer.print("选择分类关联店铺信息，无法删除！");
//		            	return;
//		            }
...
//    }
     ...
     * @return
     */
    @SecurityMapping(display = false, rsequence = 0, title = "会员ajax判断", value = "/admin/user_del.htm*", rtype = "admin", rname = "会员管理", rcode = "user_manage", rgroup = "会员")
    @RequestMapping({"/admin/user_delable.htm"})
    public void user_delable(HttpServletRequest request,HttpServletResponse response, String mulitId, String currentPage){
        String[] ids = mulitId.split(",");
        response.setContentType("text/plain");
        response.setHeader("Cache-Control", "no-cache");
        response.setCharacterEncoding("UTF-8");
            ...
            if (!id.equals("")){
                ...
                if (!parent.getUsername().equals("admin")){
                    ...
                    if (parent.getChilds() != null&&parent.getChilds().size()>0){
    		            	writer.print("所选用户已被关联，无法删除！");
    		            	return;
                    }
                    if (parent.getStore() != null||(parent.getOfs()!=null&&parent.getOfs().size()>0)){
    		            	writer.print("所选用户已被关联，无法删除！");
    		            	return;
                    }
                }
