...
import cn.wemalls.manage.buyer.action.BaseBuyerAction;
import cn.wemalls.manage.seller.action.BaseSellerAction;
import cn.wemalls.view.web.action.CartViewAction;
...
public class AdminManageAction implements ServletContextAware {
        ...
        for (String id : ids){
            if (!id.equals("")){
                ...
                if (!user.getUsername().equals("admin")){
                    this.databaseTools.execute("delete from wemalls_syslog where user_id=" + id);
                    this.databaseTools.execute("delete from wemalls_user_role where user_id=" + id);
    ...
    public String init_role(){
        ...
        if ((current_user != null) && (current_user.getUserRole().indexOf("ADMIN") >= 0) && (current_user.getUsername().equals("admin"))){
            this.databaseTools.execute("delete from wemalls_role_res");
            this.databaseTools.execute("delete from wemalls_res");
            this.databaseTools.execute("delete from wemalls_user_role");
            this.databaseTools.execute("delete from wemalls_role");
            this.databaseTools.execute("delete from wemalls_rolegroup");
