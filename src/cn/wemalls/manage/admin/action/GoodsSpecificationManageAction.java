...
import cn.wemalls.foundation.domain.GoodsSpecification;
import cn.wemalls.foundation.domain.query.GoodsSpecificationQueryObject;
...
public class GoodsSpecificationManageAction {
    ...
    public void goods_property_delete(HttpServletRequest request, HttpServletResponse response, String id){
        boolean ret = true;
        if (!id.equals("")){
            this.databaseTools.execute("delete from wemalls_goods_spec where spec_id=" +
                                       id);
            this.databaseTools.execute("delete from wemalls_cart_gsp where gsp_id=" +
                                       id);
