...
import cn.wemalls.foundation.domain.query.AccessoryQueryObject;
import cn.wemalls.foundation.domain.query.GoodsQueryObject;
import cn.wemalls.foundation.domain.query.ReportQueryObject;
import cn.wemalls.foundation.domain.query.TransportQueryObject;
...
public class GoodsSellerAction {
        ...
        if (store_status == 2){
            ...
            List ugcs = this.userGoodsClassService
                        .query("select obj from UserGoodsClass obj where obj.user.id=:user_id and obj.display=:display and obj.parent.id is null order by obj.sequence asc",
                               params, -1, -1);
            params.clear();
            params.put("userStatus", Integer.parseInt("0"));
            List gbs = this.goodsBrandService.query(
                           "select obj from GoodsBrand obj where obj.userStatus=:userStatus order by obj.sequence asc",
                           params, -1, -1);
    ...
    public ModelAndView add_goods_finish(HttpServletRequest request,
        ...
        if (goods_session1.equals("")){
            ...
            if (id.equals("")){
                ...
                String goods_lucene_path = (new StringBuilder(String.valueOf(System.getProperty("wemalls_business_edition.root")))).append(File.separator).append("lucene").append(File.separator).append("goods").toString();
    ...
    public void add_goods_class_staple(HttpServletRequest request,
                                       HttpServletResponse response,String selid){
        String ret = "error";
        if (selid!= null&&selid.length()>0){
            GoodsClass gc =goodsClassService.getObjById(Long.valueOf(selid));
            Map params = new HashMap();
            User user = this.userService.getObjById(SecurityUserHolder
                                                    .getCurrentUser().getId());
            params.put("store_id", user.getStore().getId());
    ...
    public void load_goods_class_staple(HttpServletRequest request,
                                        HttpServletResponse response, String id, String name) throws Exception{
        GoodsClass obj = null;
        if ((id != null) && (!id.equals("")))
            obj = this.goodsclassstapleService.getObjById(Long.valueOf(Long.parseLong(id))).getGc();
        if ((name != null) && (!name.equals("")))
            obj = this.goodsClassService.getObjByProperty("className", URLDecoder.decode(name,"utf-8"));
        List list = new ArrayList();
        ...
        if (obj != null){
            ...
            Map third_map = new HashMap();
            Map other_map = new HashMap();
            Object gc;
            if (obj.getLevel() == 2){
                ...
                for (GoodsClass gc1 : second_gcs){
                    ...
                    if(gc1.getId().equals(obj.getParent().getId())){
                    	params.clear();
                    	params.put("pid", obj.getParent().getId());
                    	List<GoodsClass> third_gcs = this.goodsClassService
                    			.query("select obj from GoodsClass obj where obj.parent.id=:pid order by obj.sequence asc", params, -1, -1);
                    	for (GoodsClass gc2 : third_gcs){ // gc = (GoodsClass)map.next();
                    		Map map1 = new HashMap();
                    		map1.put("id", ((GoodsClass) gc2).getId());
                    		map1.put("className", ((GoodsClass) gc2).getClassName());
                    		third_list.add(map1);
                    	}
                    	map.put("thirdclass", third_list);
                    }
                    second_list.add(map);
                    jsonMap.put("secondclass", second_list);
            ...
            jsonMap.put("staple_info", staple_info.substring(0, staple_info.length() - 1));
//            other_list.add(map);
...
//            jsonMap.put("third", );
    ...
    private String generic_goods_class_info(GoodsClass gc){
        String goods_class_info = (gc.getClassName()!=null?gc.getClassName():"") + ">";
