...
public class GoodsType extends IdEntity {
    ...
    //商品规格集合
    @ManyToMany
    @JoinTable(name = "wemalls_goodstype_spec", joinColumns = {@javax.persistence.JoinColumn(name = "type_id")}, inverseJoinColumns = {@javax.persistence.JoinColumn(name = "spec_id")})
    private List<GoodsSpecification> gss = new ArrayList();

    //商品品牌集合
    @ManyToMany
    @JoinTable(name = "wemalls_goodstype_brand", joinColumns = {@javax.persistence.JoinColumn(name = "type_id")}, inverseJoinColumns = {@javax.persistence.JoinColumn(name = "brand_id")})
